...
const androidValuesStrings =
  "android/app/src/main/res/values/strings.xml";
const androidJava = "android/app/src/*/java";
const iosXcodeproj = "ios/*.xcodeproj";
const iosPbxProject = "ios/*.xcodeproj/project.pbxproj";
const iosAppDelegate = "ios/*/AppDelegate.h";
...
const getIosFoldersAndFilesPaths = ({
  currentPathContentStr,
  newPathContentStr,
}) => {
  const cleanNewPathContentStr = utils.cleanString(newPathContentStr);
...
const getIosUpdateFilesContentOptions = ({
  ...
  const encodedNewName = utils.encodeXmlEntities(newName);
  const encodedCurrentName = utils.encodeXmlEntities(currentName);
  const cleanNewPathContentStr = utils.cleanString(newPathContentStr);
  ...
  return [
    ...
    },
    {
      ...
      from: [
        ...
        new RegExp(
          `\\bpath = ${cleanNewPathContentStr}/AppDelegate.h\\b`,
          "gi"
        ),
        new RegExp(
          `\\bpath = ${cleanNewPathContentStr}/AppDelegate.mm\\b`,
          "gi"
        ),
        new RegExp(
          `\\bpath = ${cleanNewPathContentStr}/Images.xcassets\\b`,
          "gi"
        ),
        new RegExp(`\\bpath = ${cleanNewPathContentStr}/Info.plist\\b`, "gi"),
        new RegExp(`\\bpath = ${cleanNewPathContentStr}/main.m\\b`, "gi"),
        new RegExp(
          `\\bpath = ${cleanNewPathContentStr}/LaunchScreen.storyboard\\b`,
          "gi"
        ),
    ...
    },
    {
      files: "ios/*.xcodeproj/project.pbxproj",
      processor: (input) => {
        const matchesDisplayName = input.match(
...
const getAndroidUpdateFilesContentOptions = ({
  ...
  const newModulesName = utils.cleanString(newName).toLowerCase();
...
const getAndroidUpdateBundleIDOptions = ({
  currentBundleID,
  newBundleID,
  ...
  return [
    ...
    },
    {
      ...
      from: [
        new RegExp(`L${currentBundleIDAsPath}`, "g"),
        new RegExp(`L${currentBundleID}`, "g"),
      ],
...
const getOtherUpdateFilesContentOptions = ({
  currentName,
  newName,
  currentPathContentStr,
  newPathContentStr,
  ...
  return [
    ...
    },
    {
      files: "package.json",
      from: [
        new RegExp(`${packageJsonName}`, "gi"),
        new RegExp(`${currentPathContentStr}`, "gi"),
      ],
      to: cleanNewPathContentStr,
    },
    {
