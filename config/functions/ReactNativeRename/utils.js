...
const { pathEmitter, getAppPath } = require("./state");
const { log, errLog } = require("./logger.js");
...
let APP_PATH = getAppPath(); // Initial set

// Function to call when appPath changes
function onPathChange(newPath) {
  APP_PATH = newPath;
}

// Set up the listener for path changes
pathEmitter.on("pathChange", onPathChange);
...
const cleanString = (str) => str.replace(NON_LANGUAGE_ALPHANUMERIC_REGEX, "");
const bundleIDToPath = (bundleID) => bundleID.replace(/\./g, "/");
const decodeXmlEntities = (name) => decode(name, { level: "xml" });
const encodeXmlEntities = (name) =>
  encode(name, { mode: "nonAscii", level: "xml", numeric: "hexadecimal" });
const normalizePath =
  process.platform === "win32" ? require("normalize-path") : (p) => p;
// const androidValuesStringsFullPath = path.join(APP_PATH, androidValuesStrings);

const validateCreation = async () => {
  const iosAppDelegatePaths = globSync(
    normalizePath(path.join(APP_PATH, iosAppDelegate))
  );
  const iosInfoPlistFullPath =
    iosAppDelegatePaths.length > 0
      ? iosAppDelegatePaths[0].replace("AppDelegate.h", "Info.plist")
      : null;
  const androidValuesStringsFullPath = path.join(
    APP_PATH,
    androidValuesStrings
  );
  const fileExists =
    iosInfoPlistFullPath !== null &&
    fs.existsSync(iosInfoPlistFullPath) &&
    fs.existsSync(androidValuesStringsFullPath);
  if (!fileExists) {
    log('Directory should be created using "react-native init".');
  }
  return fileExists;
};
...
const validateNewName = (newName, programOptions) => {
  if (!(newName.length <= MAX_NAME_LENGTH)) {
    log(`New app name "${newName}" is too long`);
    process.exit();
  }

  const cleanNewName = cleanString(newName);
  const isCleanNewNameLengthValid =
    cleanNewName.length >= MIN_LANGUAGE_ALPHANUMERIC_NAME_LENGTH;
...
const validateNewPathContentStr = (value) => {
  const cleanValue = cleanString(value ?? "");
  const isCleanValueLengthValid =
    cleanValue.length >= MIN_LANGUAGE_ALPHANUMERIC_NAME_LENGTH;
...
const validateNewBundleID = (newBundleID, platforms = []) => {
  const isIosBundleIdValid = VALID_IOS_BUNDLE_ID_REGEX.test(newBundleID);
  const isAndroidBundleIdValid =
    VALID_ANDROID_BUNDLE_ID_REGEX.test(newBundleID);
  ...
  if (
    ...
    log(errorMessage);
    log(chalk.yellow(additionalMessage));
    process.exit();
  }
...
const getIosCurrentName = () => {
  ...
  const filepath =
    iosAppDelegatePaths.length > 0
      ? iosAppDelegatePaths[0].replace("AppDelegate.h", "Info.plist")
      : null;
  const fileExists = filepath !== null && fs.existsSync(filepath);
  if (fileExists) {
...
const getAndroidCurrentName = () => {
  ...
  const fileExists = fs.existsSync(gradleFile);
  if (!fileExists) {
    errLog(
      `Unable to get project name from settings.gradle for project ${APP_PATH}`
    );
  } else {
    const gradleFileContent = fs.readFileSync(gradleFile, "utf8");
    const projectName = gradleFileContent.match(
...
const getAndroidCurrentBundleID = () => {
  ...
  const gradleFileContent = fs.readFileSync(gradleFile, "utf8");
  const bundleIDFromGradle = gradleFileContent.match(
    /applicationId\s+['"](.+)['"]/
  )[1];
  ...
  errLog(
    `Unable to get bundleID from manifest or gradle file for project ${APP_PATH}`
  );
};
...
const getIosXcodeProjectPathName = () => {
  const xcodeProjectPath = globSync(
    normalizePath(path.join(APP_PATH, iosXcodeproj)),
    {
...
const renameFoldersAndFiles = async ({
  ...
  const promises = foldersAndFilesPaths.map(async (filePath, index) => {
    ...
    const newPath = path.join(
      APP_PATH,
      filePath.replace(currentPathParam, newPathParam)
    );

    if (currentPath === newPath) {
      return log(toRelativePath(currentPath), chalk.yellow("NOT RENAMED"));
    }
    ...
    try {
      new Promise((resolve) => {
        ...
        const shellMove = shell.mv(
          "-f",
          `${currentPath}${shellMoveCurrentPathGlobEnd}`,
          newPath
        );

        if (shellMove.code !== 0) {
          log(chalk.red(shellMove.stderr));
        }

        resolve();
        log(toRelativePath(newPath), chalk.green("RENAMED"));
      });
    } catch (error) {
      log(error);
    }
  ...
  const promises = filesContentOptions.map(async (option, index) => {
    ...
    try {
      ...
      const filePath = error.message
        .replace("No files match the pattern:", "")
        .trim();
      log(toRelativePath(filePath), chalk.yellow("NOT FOUND"));
    }
  });
...
const updateElementInXml = async ({ filepath, selector, text }) => {
  ...
  log(toRelativePath(filepath), chalk.green("UPDATED"));
};

const updateIosNameInInfoPlist = async (newName) => {
...
const updateAndroidNameInStringsXml = async (newName) => {
  const androidValuesStringsFullPath = path.join(
    APP_PATH,
    androidValuesStrings
  );
...
const renameAndroidBundleIDFolders = async ({
  currentBundleIDAsPath,
  newBundleIDAsPath,
}) => {
  const currentBundleIDFoldersRelativePaths = globSync(
    normalizePath(path.join(APP_PATH, `${androidJava}`)),
    { onlyDirectories: true }
  ).map((folderPath) =>
    normalizePath(toRelativePath(`${folderPath}/${currentBundleIDAsPath}`))
  );
...
const updateAndroidFilesContent = async ({
  currentName,
  newName,
  newBundleIDAsPath,
}) => {
...
const updateAndroidFilesContentBundleID = async ({
  currentBundleID,
  newBundleID,
  currentBundleIDAsPath,
  newBundleIDAsPath,
...
const updateOtherFilesContent = async ({
  newName,
  currentPathContentStr,
  newPathContentStr,
  currentIosName,
...
const cleanBuilds = () => {
  ...
  log(chalk.yellow("Done removing builds."));
};

const showSuccessMessages = (newName) => {
  const appJsonContent = getJsonContent(appJson);
...
${chalk.green(
  "SUCCESS! 🎉 🎉 🎉"
)} Your app has been renamed to "${chalk.blue(newName)}".
`
  );
...
`)
  ...
  log(
    chalk.black(`- Please make sure to run "npx pod-install" and "watchman watch-del-all" before running the app.

If you like this tool, please give it a star on GitHub: https://github.com/bigegu24/react-native-rename-gui

`)
  );
};

const gitStageChanges = () => {
  shell.cd(APP_PATH);
...
const checkPackageUpdate = async () => {
  try {
    ...
    if (res?.latest) {
      log();
      log(chalk.green.bold(`A new version of "${pjson.name}" is available.`));
      log("Current version:", chalk.yellow(pjson.version));
      log("Latest version:", chalk.yellow(res.latest));
      log(
        chalk.cyan(`You can update by running: npm install -g ${pjson.name}.`)
      );
      log();
    }
  } catch (error) {
